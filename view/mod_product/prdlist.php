...
@$category->name=replacesitekey(MAINKEY,SITEKEY,DOMAINKEY,$category->name);
?>
	...
	<div class="prod_list_con">
        ...
        if (sizeof($products) > 0) {
            ...
            foreach ($products as $product) {
				...
				$product->name=replacesitekey(MAINKEY,SITEKEY,DOMAINKEY,$product->name);
				$product->description=replacesitekey(MAINKEY,SITEKEY,DOMAINKEY,$product->description);
        ?>
			<div class="prod_list_list">
				<div class="prod_list_pic"><a href="<?php echo Html::uriquery('mod_product', 'view', array('p_id' => $product->id)); ?>" title="<?php echo $product->name; ?>"><img name="picautozoom" src="<?php echo $product->feature_smallimg; ?>" alt="<?php echo $product->name; ?>" border="0" /></a></div>
				<div class="prod_list_name"><a href="<?php echo Html::uriquery('mod_product', 'view', array('p_id' => $product->id)); ?>" title="<?php echo $product->name; ?>"><?php echo $product->name; ?></a></div>
				<?php
					...
					$product_ProductCategory_name=replacesitekey(MAINKEY,SITEKEY,DOMAINKEY,$product_ProductCategory_name);
				?>
				...
				<div class="prod_list_intr"><?php echo Toolkit::substr_MB(strip_tags($product->description), 0, 72).((Toolkit::strlen_MB(strip_tags($product->description))>72)?'...':''); ?></div>
			</div>
			<?php
                $row_idx = 1 - $row_idx;
            }
