...
$showType = unserialize($qqs->s_param);
var_dump($qqs);*/
$curr_mblock = new ModuleBlock();
$curr_sour = $curr_mblock->find('module="mod_qq"');
$curr_style= unserialize($curr_sour->s_param);
$curr_style['qq_show_type'] = isset($curr_style['qq_show_type'])?$curr_style['qq_show_type']:'1';
/*
 * 浮动客服
 * for sitestarv1.3
 */
if ((SessionHolder::get('page/status', 'view') == 'view') && (QQ_ONLINE == '1')) {
	if (isset($_SITE->s_locale)&&$_SITE->s_locale=='zh_CN') {
		if (QQ_ONLINE_POS == 'left') {
			$head_image = 'head_'.$curr_style['qq_show_type']."_c.gif";
			$l_type_image = 'l_bar_'.$curr_style['qq_show_type']."_c.gif";
			$zxkfCss = <<<CSS
		.zxkf_head {background:url(images/{$head_image}) no-repeat;}
		.zxkf_obtn {margin-top:0;width:36px;background:url(images/{$l_type_image}) no-repeat;float:left;height:155px;margin-right:-5px;_width:41px;}
CSS;
		} else {
			$head_image = 'head_'.$curr_style['qq_show_type']."_c.gif";
			$r_type_image = 'r_bar_'.$curr_style['qq_show_type']."_c.gif";
			$zxkfCss = <<<CSS
	html {overflow-x:hidden;}
	.zxkf_head {background:url(images/{$head_image}) no-repeat;}
	.zxkf_obtn {margin-top:0;width:36px;background:url(images/{$r_type_image}) no-repeat;float:left;height:155px;margin-right:-5px;_width:41px;}
CSS;
		}
	}else{//如果是英文
		if (QQ_ONLINE_POS == 'left') {
			$head_image = 'head_'.$curr_style['qq_show_type']."_e.gif";
			$l_type_image = 'l_bar_'.$curr_style['qq_show_type']."_e.gif";
			$zxkfCss = <<<CSS
	.zxkf_head {background:url(images/{$head_image}) no-repeat;}
	.zxkf_obtn {margin-top:0;width:36px;background:url(images/{$l_type_image}) no-repeat;float:left;height:155px;margin-right:-5px;_width:41px;}
CSS;
			...
			$head_image = 'head_'.$curr_style['qq_show_type']."_e.gif";
			$r_type_image = 'r_bar_'.$curr_style['qq_show_type']."_e.gif";
			$zxkfCss = <<<CSS
	html {overflow-x:hidden;}
	.zxkf_head {background:url(images/{$head_image}) no-repeat;}
	.zxkf_obtn {margin-top:0;width:36px;background:url(images/{$r_type_image}) no-repeat;float:left;height:155px;margin-right:-5px;_width:41px;}
  ...
  <tr>
    <td class="zxkf_info" valign="top" align="center">
      <table class="zxkf_qqtable" border="0" cellspacing="0" cellpadding="0" width="135" align="center">
    	...
    	?>
    	<tr>
          <td height="30" align="center"><span><img src="http://wpa.qq.com/pa?p=2:<?php echo $online->account; ?>:17" align="absMiddle" alt="QQ" border="0" />&nbsp;&nbsp;<a href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo $online->account; ?>&site=qq&amp;Menu=yes" target=blank><?php echo $online->qqname; ?></a></span></td></tr>
    	<?php
    				break;
    			case '1': // MSN
        ?>
        ...
        ?>
        <tr>
          <td height="30" align="center"><span><a href="callto://<?php echo $online->account; ?>" title="<?php echo $online->account; ?>" target="_blank"><img border="0" src="./images/skypelogo.gif" /></a></span></td></tr>
		<?php
				break;
        		case '4':
        ?>
        <tr>
          ...
          <a href="icq:account?email=<?php echo $online->account; ?>" target="_blank"  title="<?php echo $online->account; ?>" ><img border="0" src="images/icq.jpeg" height="25" width="25" /><?php echo $online->qqname; ?></a>
          </span></td></tr>
		<?php        			
        			break;
        		case '5':
        ...
        <tr>
          ...
          <a href="ymsgr:sendIM?<?php echo $online->account; ?>" target="_blank"  title="<?php echo $online->account; ?>" ><img border=0 src="http://opi.yahoo.com/online?u=linwang31@yahoo.com&m=g&t=1&l=cn"></a>
         </span></td></tr>
		<?php       			 
					break;
			}
