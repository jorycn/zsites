...
$o_download = new Download();
if (ACL::requireRoles(array('admin'))) {
$downloads = $o_download->findAll("published='1' AND s_locale=? ",array($curr_locale),
"ORDER BY `create_time` DESC LIMIT ".$list_size);
}else {
$downloads = $o_download->findAll("published='1' AND s_locale=? AND for_roles LIKE ? ",array($curr_locale,'%'.$user_role.'%'),
"ORDER BY `create_time` DESC LIMIT ".$list_size);
}
