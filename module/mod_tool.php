...
class ModTool extends Module {
    ...
    public function img_edit_update() {
        ...
        if (!empty($file_info["name"])) {
            if (!preg_match('/\.(' . PIC_ALLOW_EXT . ')$/i', $file_info["name"])) {
                ...
                Content::redirect(Html::uriquery('mod_tool', 'img_edit', array(
                    'id' => $img_id
                )));
            }
            if (file_exists(ROOT . '/file/' . THISSITE . '/image/' . $file_info["name"])) {
        ...
        try {
            if (!empty($file_info["name"])) {
                $img_info['img_name'] = 'file/' . THISSITE . '/image/' . $file_info["name"];
            }
            $img_info['img_type'] = '1';
        ...
        Content::redirect(Html::uriquery('mod_tool', 'new_mblock_s2', array(
            "widget" => "mod_marquee-marquee"
        )));
    }
    ...
    public function img_delete() {
        ...
        Content::redirect('index.php?' . Html::xuriquery('mod_tool', 'new_mblock_s2', array(
            "widget" => "mod_marquee-marquee"
        )));
    }
    public function img_create() {
        ...
        try {
            $mod_marquee['img_name'] = 'file/' . THISSITE . '/image/' . $file_info["name"];
            ...
            if (!$prd_id[$i]) {
                ...
                if ($html_file['error'][$i] > 0) {
                    ...
                    if (move_uploaded_file($html_file['tmp_name'][$i], ROOT . '/file/' . THISSITE . '/image/' . $rand_fname)) {
                        ...
                        $pic_arr['marquee_type'] = 'pic';
                        $pic_arr['title'] = '';
                        $pic_arr['pic'] = 'file/' . THISSITE . '/image/' . $rand_fname;
                        $pic_arr['flag'] = 1;
                        $pic_arr['link'] = $prd_name[$i];
                        $ext_prd_pic = new Marquee($prd_id[$i]);
                        $ext_prd_pic->set($pic_arr);
