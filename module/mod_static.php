...
class ModStatic extends Module {
    ...
    public function view() {
        ...
        try {
            ...
            $curr_locale = trim(SessionHolder::get('_LOCALE'));
            $count_num = $o_scontent->count("s_locale=?", array(
                $curr_locale
            ) , "ORDER BY `id` DESC");
            if ($count_num < "2") {
            ...
            if (ACL::requireRoles(array(
                ...
                $curr_scontent = & $o_scontent->find("`id`=? AND s_locale=?", array(
                    $sc_id,
                    $curr_locale
                ));
            } else {
                $curr_scontent = & $o_scontent->find("`id`=? AND for_roles LIKE ? AND s_locale=?", array(
                    $sc_id,
                    '%' . $user_role . '%',
                    $curr_locale
                ));
            }
            $source_data = & Pager::pageByText($curr_scontent->content, array(
                'sc_id' => $sc_id
            ));
            ...
            $this->assign('page_title', $page_cat);
    ...
    public function company_intro() {
        ...
        try {
            ...
            $curr_locale = trim(SessionHolder::get('_LOCALE'));
            $count_num = $ot_staticontent->count("s_locale=?", array(
                $curr_locale
            ) , "ORDER BY `id` DESC");
            if ($count_num < "2") {
                echo "<script>alert('" . __("Page data has error,rebulid it please!") . "');</script>";
            }
            $staticontent_data = $ot_staticontent->findAll("s_locale=? AND published='1'", array(
                $curr_locale
            ) , "ORDER BY `id`  LIMIT 2");
            ...
            if (ACL::requireRoles(array(
                ...
                $curr_scontent = & $o_scontent->find("`id`=? AND " . "published='1' AND s_locale=?", array(
                    $company_id,
                    $curr_locale
                ));
            } else {
                $curr_scontent = & $o_scontent->find("`id`=? AND " . "published='1' AND for_roles LIKE ? AND s_locale=?", array(
                    $company_id,
                    '%' . $user_role . '%',
                    $curr_locale
                ));
