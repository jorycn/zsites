...
$search_where .= ') AND article_category_id <> 0';
...
array($article_id,$now,$now,$now,$now,$curr_locale));
if($curr_article){
$curr_article->v_num++;
$curr_article->save();
}else{
...
array($article_id,$now,$now,$now,$now,'%'.$user_role.'%',$curr_locale));
...
$arr2 = array();
$o_article = new Article();
$articles =&$o_article->findAll(" published='1' AND article_category_id=".$article_category_id." AND s_locale=?",array( $curr_locale));
