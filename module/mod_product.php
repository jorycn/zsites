...
{
    protected $_filters = array('check_login' => '{newprd}{prdlist}{view}{recmndprd}');
    private $stack = array();
    ...
    public function prdlist()
    {
        ...
        $title_info            = $page_title->find(" `link`=?  and  s_locale=?", array(
            "_m=mod_product&_a=prdlist",
            $curr_locale
        ), " limit 1");
        $search_where          = '';
        $search_params         = array();
        ...
        try {
            ...
            if (ACL::requireRoles(array(
                ...
                $product_data =& Pager::pageByObject('Product', "((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND s_locale=?" . $search_where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
                    $curr_locale
                ), $search_params), "ORDER BY `i_order` DESC, `create_time` DESC");
            } else {
                $product_data =& Pager::pageByObject('Product', "((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND for_roles LIKE ? AND s_locale=?" . $search_where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
    ...
    public function view()
    {
        ...
        try {
            ...
            if (ACL::requireRoles(array(
                ...
                $curr_product =& $o_product->find("`id`=? AND " . "((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND s_locale=?", array(
                    $p_id,
                    $now,
                    $now,
                    $now,
                ...
                $curr_product =& $o_product->find("`id`=? AND " . "((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND for_roles LIKE ? AND s_locale=?", array(
                    $p_id,
                    $now,
                    $now,
                    $now,
            ...
            $curr_product->loadRelatedObjects(REL_PARENT, array(
                'ProductCategory'
            ));
            $page_title = new MenuItem();
            if ($page_title->count(" `link`=?  and  s_locale=?", array(
                "_m=mod_product&_a=view&p_id={$p_id}",
                $curr_locale
            ))) {
                $title_info = $page_title->find(" `link`=?  and  s_locale=?", array(
                    "_m=mod_product&_a=view&p_id={$p_id}",
            ...
            $description =& Pager::pageByText($content, array(
                'p_id' => $p_id
            ));
    ...
    private function _getNewPrds($list_size, $cap_id)
    {
        ...
        try {
            ...
            if (ACL::requireRoles(array(
                ...
                $products =& $o_product->findAll("((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND s_locale=?" . $where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
                    $curr_locale
                ), $params), "ORDER BY `i_order` DESC,`create_time` DESC LIMIT " . $list_size);
            } else {
                $products =& $o_product->findAll("((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND for_roles LIKE ? AND s_locale=?" . $where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
    ...
    private function _getRecmndPrds($list_size, $cap_id)
    {
        ...
        try {
            ...
            if (ACL::requireRoles(array(
                ...
                $products =& $o_product->findAll("((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND s_locale=? AND " . "recommended='1'" . $where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
                ...
                $products =& $o_product->findAll("((`pub_start_time`<? AND `pub_end_time`>=?) OR " . "(`pub_start_time`<? AND `pub_end_time`='-1') OR " . "(`pub_start_time`='-1' AND `pub_end_time`>=?) OR " . "(`pub_start_time`='-1' AND `pub_end_time`='-1')) AND " . "published='1' AND for_roles LIKE ? AND s_locale=? AND " . "recommended='1'" . $where, array_merge(array(
                    $now,
                    $now,
                    $now,
                    $now,
    ...
    private function getCategoryChildIds($parentid, $curr_locale)
    {
        ...
        $product_category        = new ProductCategory();
        $product_childcategories = $product_category->findAll("product_category_id IN({$parentid}) AND s_locale = '{$curr_locale}'");
        $childids                = '';
        if (count($product_childcategories)) {
    ...
    private function getNextAndPrev($id, $product_category_id)
    {
        ...
        $arr2        = array();
        $o_product   = new Product();
        $prods =& $o_product->findAll(" published='1' AND product_category_id=" . $product_category_id . " AND s_locale=?", array(
            $curr_locale
        ));
        ...
        @$str .= is_string($prev['str']) ? $prev['str'] : "<a href=" . Html::uriquery("mod_product", "view", array(
            'p_id' => $prev['id']
        )) . ">" . $prev['name'] . "</a><br>";
        $str .= '<font style="color:#595959">' . __('Next product') . '</font>：';
        $str .= is_string($next['str']) ? $next['str'] : "<a href=" . Html::uriquery("mod_product", "view", array(
            'p_id' => $next['id']
        )) . ">" . $next['name'] . "</a></div>";
        return $str;
    }
