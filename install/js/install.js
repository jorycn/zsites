...
jQuery(function($){
	...
	$("#js-install-at-once").click( function () {
		...
		var s =document.getElementById("sitename").value;
		if(s=="")
		{
			 alert("请输入正确的网站名称");
		...
		function obj(){
			...
			this.resend = function(){
				...
				$.ajax({
					type: "POST",
					url: "index.php",
					data: "db_host="+$("#db-host").val()+"&db_port="+$("#db-port").val()+"&db_user="+$("#db-user").val()+"&db_pwd="+$("#db-pwd").val()+"&db_name="+$("#db-name").val()+"&db_prefix="+$("#db-prefix").val()+"&admin_name="+$("#js-admin-name").val()+"&admin_pwd="+$("#js-admin-password").val()+"&_a="+step_a+"&step="+step+"&demo=1&sitename="+$("#sitename").val()+"",
