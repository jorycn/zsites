...
<title><?php _e('Media File Manager');?></title>
...
while($file = $dh->read()) {
	...
	else if($file == "..")
	{
		...
		$tmp = $path['dirname'];
		$line = "\n<tr>
    <td class='linerow'> <a href=select_media.php?f=$f&activepath=".urlencode($tmp)."><img src=img/dir2.gif border=0 width=16 height=16 align=absmiddle>".__('Parent directory')."</a></td>
...
$dirlist = $dirs = array();
$base_upload_dir = 'file/'.THISSITE.'/media/';
...
<td background="img/tbg.gif" bgcolor="#99CC00">
  &nbsp;<?php _e('Upload');?>： <input type='file' name='uploadfile' style='width:320px' title="<?php _e('Supported file formats');?>：swf|mpg|mp3|rm|rmvb|wmv|wma|wav|mid|mov" /><select name="mediadir" id="gtcurdir">
  <?php
