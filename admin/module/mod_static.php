		...
		if(!empty($banner_file['name']))
		{
			...
			if($banner_file['name']) {
				...
				if($flag_flash)
				{
					...
					$banner_arr['flv_src'] = 'file/'.THISSITE.'/flash/'.$banner_file["name"];
				}
				else
				{
					if(file_exists(ROOT.'/file/'.THISSITE.'/image/'.$banner_file["name"])) {
						$banner_file["name"] = Toolkit::randomStr(8).strrchr($banner_file["name"],".");
					}
					...
					$banner_arr['img_src'] = 'file/'.THISSITE.'/image/'.$banner_file["name"];
				}
			} else {
...
//				Content::redirect(Html::uriquery('mod_static', 'admin_mod'));
			...
			if(file_exists(ROOT.'/file/'.THISSITE.'/media/'.$music_file["name"])) {
			...
			$music_arr['BG_MUSIC'] = 'file/'.THISSITE.'/media/'.$music_file["name"];
