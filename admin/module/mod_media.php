...
$logo_arr['img_src'] = 'file/'.THISSITE.'/image/'.$logo_file["name"];
...
if($flag_flash)
{
if(file_exists(ROOT."/file/".THISSITE."/flash/".$banner_file["name"])) {
...
$banner_arr[$current_url_str]['flv_src'] = 'file/'.THISSITE.'/flash/'.$banner_file["name"];
...
$struct_file['name'] = Toolkit::changeFileNameChineseToPinyin($struct_file['name']);
$struct_file['name'] = iconv("UTF-8","gb2312",$struct_file['name']);
return move_uploaded_file($struct_file['tmp_name'],ROOT.'/file/'.THISSITE.'/flash/'.$struct_file['name']);
...
private function img_restruck($imgfile_name,$path = 'wuyong/image/') {
define('SSFCK',1);
define('SSROOT',ROOT);
include_once(P_LIB.'/image.func.php');
$fullfilename = SSROOT."/file/".THISSITE."/image/".$imgfile_name;
