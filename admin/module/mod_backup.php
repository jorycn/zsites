...
$this->assign( 'list',$this->get_file_list("../file/".THISSITE."/backup") );
}
public function admin_load()
{
$files = $this->get_file_list("../file/".THISSITE."/backup");
$fid =&ParamHolder::get('_fid',array());
$file = "../file/".THISSITE."/backup/".$files[$fid]['fname'];
...
@chmod('../file/'.THISSITE.'/backup/',0755);
$file_name = '../file/'.THISSITE.'/backup/'.$backup['file_name'];
try{
...
if (1==2){
$pre=SITEPREFIX;
$sql="show tables  like '%$pre%'";
$res = mysql_query($sql);
$tables = array();$sql = "";
while ($result = mysql_fetch_array($res,MYSQL_NUM)) {
}
}
$tables=explode(",",getalltable(0));
...
$o_backup = new Backup();
$time = time();
$file_name = ROOT."/'file/'.THISSITE.'/backup/'{$backup['file_name']}";
...
$backup = "../file/".THISSITE."/backup/".$files[$fid]['fname'];
...
$files = $this->get_file_list("../file/".THISSITE."/backup");
$fid =&ParamHolder::get('_fid','');
$import_file = "../file/".THISSITE."/backup/".$files[$fid]['fname'];
