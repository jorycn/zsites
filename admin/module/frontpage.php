...
class Frontpage extends Module {
    ...
    public function dashboard() {
        ...
        $cate_a_count = $o_article_category->count("id<>'1' AND s_locale=?", array(
            $curr_locale
        ));
        $o_article = new Article();
        $article_count = $o_article->count("s_locale=?", array(
            $curr_locale
        ));
        $o_prod_category = new ProductCategory();
        $cate_p_count = $o_prod_category->count("id<>'1' AND s_locale=?", array(
            $curr_locale
        ));
        $o_product = new Product();
        $prod_count = $o_product->count("s_locale=?", array(
            $curr_locale
        ));
    ...
    public function auto_upgrade() {
        try {
            ...
            while (!feof($remote_file)) {
                fwrite($local_file, fread($remote_file, 4096) , 4096);
            }
            fclose($local_file);
            fclose($remote_file);
    ...
    public function get_version() {
        try {
            ...
            if (($newvn = @file_get_contents($url)) === false) {
                ...
                $this->setVar('json', Toolkit::jsonOK(array(
                    'vn' => $newvn,
                    'curvn' => SYSVER,
                    'tag' => $status
                )));
    ...
    private function update_file($topath, $replace) {
        ...
        foreach ($files as $file) {
            if ((strtolower(substr(strrchr($file, '.') , 1)) == 'txt') || (strtolower(substr(strrchr($file, '.') , 1)) == 'sql') || ($file == $topath . '/index.php')) {
                continue;
            } else {
    ...
    public function dologin() {
        ...
        if (ACL::loginUser(ParamHolder::get('login_user', '') , ParamHolder::get('login_pwd', ''))) {
            if (!ACL::requireRoles(array(
                'admin'
            ))) {
    ...
    private function createtable($sql, $dbcharset) {
        ...
        return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql) . ($this->version() > '4.1' ? " ENGINE=$type default CHARSET=" . $dbcharset : " TYPE=$type");
    }
