...
$(document).ready(function(){
	$("#getParams1").val(parent.$("#getParams").attr('value'));
});

function showAll()
{
	var pos;
	pos = parent.$('#banner').offset();
	pos_left = pos.left-20;
	//parent.$('.ui-dialog').css({position:"absolute",left:pos_left+'px'});
	//parent.$('.ui-dialog').css('width',parent.$('#banner').width()+140);
	//parent.$('#showContents').attr('width',parent.$('#banner').width()+120);
	$("#allshow").val('yes');
	$("#custom_banner").remove();
	$.ajax({
		type: "POST",
		url:  "index.php?_m=mod_media&_a=admin_banner1",
...
<?php
$url_info = isset($_GET['_url'])?urldecode($_GET['_url']):'';
$notice_str2 = __('Edit every page banner');
echo "<div id='edit_area'><div id='custom_banner'>";
if(!(!empty($p_banner[$url_info]['img_src']) || !empty($p_banner[$url_info]['flv_src']))) 
{
	$notice_str1 = __('This page have not own banner,system load a banner that shows at every pages.');
	echo "<div style='margin-left:15px;margin-top:5px;'><img width='20' height='20' src='images/warning.gif' alt='warning'/>$notice_str1&nbsp;&nbsp;<a href='#' onclick='showAll();'><font style='font-weight:bold;'>$notice_str2</font></a></div>";
}
else
{
	echo "<div style='margin-left:15px;margin-top:5px;margin-bottom:10px;'><a href='#' onclick='showAll();'><font style='font-weight:bold;'>$notice_str2</font></a></div>";
}
//$sinfo_form = new Form('index.php', 'sinfoform', 'check_sinfo_info');
//$sinfo_form->setEncType('multipart/form-data');
//$sinfo_form->p_open('mod_media', 'save_banner');

if(isset($p_banner[$url_info])){$p_banner_url_info=$p_banner[$url_info];}
$lhtype='';
...
<input type="hidden" name="_m" id="_m" value="mod_media"  /><input type="hidden" name="_a" id="_a" value="save_banner"  /><input type="hidden" name="_r" id="_r" value="_page"  />
...
<table id="sinfoform_table" class="form_table" width="100%" border="0" cellspacing="0" cellpadding="2" style="line-height:24px;">
	...
	<tbody>
		<?php
          ...
          if(isset($del_banner_img)){  echo Html::input('hidden', 'param[del_banner_img]', $del_banner_img);}
		?>
        <tr>
...
$single_banner = $p_banner['_all'];
        ...
        <tr id="img_link" style="display:<?php if($geshi==3){echo '';}else{echo 'none';}?>">
        ...
        <tr id=img_link_open  style="display:<?php if($geshi==3){echo '';}else{echo 'none';}?>">
            <td class="label"><?php echo __("open image in window from self or blank"); ?></td>
            <td class="entry"   style="text-align:left" ><input type="radio" name="img_link_open" value="1" /><?php echo __("image self"); ?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="img_link_open" value="2" />&nbsp;&nbsp;&nbsp;<?php echo __("image blank"); ?> 
            </td>
        ...
        <tr  id="flashfile" style="display:<?php if($geshi==2){echo '';}else{echo 'none';}?>">
         <td class="label"><?php echo __('Swf file');?></td>
            <td class="entry">          
            <input type="file" name="banner_file" id="banner_file" value=""  /> <?php if($flv_src){echo ' √';}else{echo ' X';} //修改flash路径问题 未解决?>  <BR />√:<?php echo __('File note yes');?>  X:<?php echo __('File note no');?>&nbsp;&nbsp; </td>
          ...
          <?php if(!$lhtype){$lhtype=4;}?>
			...
			?>
			  ...
			  </script> </div>  <div id="addclick" style=" float:left;<?php if($geshi=='2') echo 'display:none'; ?>">   <a href="javascript:void(0);" onClick="additem();return false;"><?php echo __('Add Slide items');?></a></div>
              </td></tr>
            <tr>
            <td class="label"><?php _e('Size'); ?></td>
            <td class="entry">
            <?php 
			if(isset($p_banner[$url_info]['img_width'])&&$p_banner[$url_info]['img_width']==0){$p_banner[$url_info]['img_width']=$def_width;}
			if(isset($p_banner[$url_info]['img_height'])&&$p_banner[$url_info]['img_height']==0){$p_banner[$url_info]['img_height']=round($def_width/3,0);}
         //  echo Html::input('text', 'param[banner_width]', $p_banner[$url_info]['img_width'], 
           //     'class="textinput" style="width:40px;"', $mod_form);
			if(isset($p_banner[$url_info]['img_width'])){	  echo Html::input('text', 'param[banner_width]', $p_banner[$url_info]['img_width'], 
                'class="textinput" style="width:40px;"', $mod_form);}
				else{
				 echo Html::input('text', 'param[banner_width]',$def_width, 
                'class="textinput" style="width:40px;"', $mod_form);
				}
            ?>
            &times;
            <?php
         if(isset($p_banner[$url_info]['img_height'])){  
		 			 echo Html::input('text', 'param[banner_height]', $p_banner[$url_info]['img_height'], 
                'class="textinput" style="width:40px;"', $mod_form);
				}else{
					 echo Html::input('text', 'param[banner_height]', round(($def_width/3),0), 
                'class="textinput" style="width:40px;"', $mod_form);
				}
            ?>         <?php echo __('Is recommend');?>：<?php echo __('Width');?> <?php echo $def_width;?>px &nbsp;&nbsp; <?php echo __('Height');?> <?php echo round(($def_width/3),0);?>px 
            
            <div id="show_banner_note"  style="display:<?php if($geshi==2){echo 'none';}else{echo '';}?>"><?php _e("Banner note");?></div>
</td>
        </tr>
       ...
       <?php
         ...
         <tr><td  class="label"><?php echo __('Link mode');?></td><td class="entry"  style="text-align:left">
          ...
          <tr id=banner_link_open<?php echo $kk; ?>   <?php if(isset($isLink[$kk-1])&&$isLink[$kk-1]=='no'){ ?> style="display:none;" <?php } ?>>
            <td class="label"><?php echo __("open image in window from self or blank"); ?></td>
            <td class="entry"   style="text-align:left" ><input type="radio" name="link_open<?php echo $kk;?>[]" id="ex_params_image_open_" value="1" <?php if(!empty($img_open_type)&&isset($img_open_type[$kk][0])&&$img_open_type[$kk][0]==1){echo "checked";}?> /><?php echo __("image self"); ?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="link_open<?php echo $kk;?>[]" id="ex_params_image_open_" value="2" <?php if(!empty($img_open_type)&&isset($img_open_type[$kk][0])&&$img_open_type[$kk][0]==2){echo "checked";}?> />&nbsp;&nbsp;&nbsp;<?php echo __("image blank"); ?>            </td>
