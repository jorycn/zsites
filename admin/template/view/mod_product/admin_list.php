...
function on_del_failure(response) {
    document.getElementById("adminprdlst_stat").innerHTML = "<?php
_e('Request failed!'); ?>";
    return false;
}
...
function delete_product(p_id) {
    if (confirm("<?php
_e('Delete the selected product?'); ?>")) {
        var stat = document.getElementById("adminprdlst_stat");
        stat.style.display = "block";
        stat.innerHTML = "<?php
_e('Deleting selected product...'); ?>";
...
function delete_products(){
    ...
    if(str.length < 1) {
        alert("<?php
_e('Please select items to be deleted!'); ?>");
    } else {
        delete_product(str);
    }
}
...
?>  
        <li><a class="iconfl" href="<?php
    echo Html::uriquery('mod_category_p', 'admin_list', array(
        'goto' => '10000'
    )); ?>" title=""><?php
    _e('Manage Categories'); ?></a></li>
<?php
}
?>  
 <?php
...
?>     
        <li><a class="icontj" href="<?php
    echo Html::uriquery('mod_product', 'admin_add'); ?>" title=""><?php
    _e('Add Product'); ?></a></li>
<?php
}
?>
...
?>          
        <li><a class="icontj" href="<?php
    echo Html::uriquery('mod_product', 'admin_batch'); ?>" title=""><?php
    _e('Batch Import'); ?></a></li>
<?php
}
?>  
<?php
...
?><a class="icontj" href="javascript:void(0);" onclick="loadCsv();return false;" title=""><?php
    _e('Batch Export'); ?></a>
    <?php
    $batch_export_form->close(); ?></li>
<?php
}
?>  
<?php
...
?>      
        <li><a class="iconsc" href="javascript:void(0)" onclick="delete_products();"><?php
    _e('Delete Selected'); ?></a></li>
<?php
}
?>  
        <li style="margin-top:14px;margin-right:15px;"><?php
include_once (dirname(__FILE__) . '/_category_switch.php'); ?></li>
    <li style="margin-top:14px;margin-right:5px;"><?php
include_once (P_TPL . '/common/language_switch.php');
if (SessionHolder::get('_LOCALE', DEFAULT_LOCALE) != 'en') { ?></li><li style="margin-top:16px;line-height:0px;">
    <img id="answer" class="title" src="<?php
    echo P_TPL_WEB; ?>/images/answer1.gif" alt="help" title="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php
    _e('Language note'); ?>"/>
    <?php
} ?></li>
<?php
...
?>  
    <li><input style="_margin-top:10px;margin-top:10px;width:70px;_width:70px;" type="submit" value="<?php
    _e('Save Order'); ?>" id="submit" name="submit"/></li>
<?php
}
?>      
    <li style="margin-top:0;_margin-top:14px;"><?php
echo Html::input('text', 'keyword', $keyword, 'class="textinput" style="width:70px;"');
echo '&nbsp;&nbsp;' . Html::input('button', 'search', '', ' class="btn" onclick="keyword_search()"'); ?></li>
</ul>
...
<table class="form_table_list" id="admin_product_list" width="100%" border="0" cellspacing="1" cellpadding="2" style="line-height:24px;">
    <thead>
        <tr>
            <th width="10%"><?php
echo Html::input('checkbox', 'ckselect', '', 'onclick=ck_select()'); ?></th>
            <th><?php
_e('Name'); ?></th>
            <th><?php
_e('Category'); ?></th>
...
_e('Order'); ?></th>
            <th><?php
_e('Show at frontpage'); ?></th>
            <th><?php
_e('Recommend'); ?></th>
            <th width="70"><?php
_e('Member only access'); ?></th>
            <th width="70"><?php
_e('Online Orderable'); ?></th>
            <th><?php
_e('Publish Date'); //_e('Create Time');
 ?></th>
            <th><?php
_e('Edit'); ?></th>
            <th><?php
...
if (sizeof($products) > 0) {
    ...
    foreach ($products as $product) {
        $product->loadRelatedObjects(REL_PARENT, array(
            'ProductCategory'
        ));
?>
        <tr class="row_style_<?php
        echo $row_idx; ?>" valign="middle">
            <td><?php
        echo Html::input('checkbox', 'product', $product->id); ?></td>
            <td>
             <?php
        if ($lang_sw == $default_lang) {
?>
            <a href="../<?php
            echo Html::uriquery('mod_product', 'view', array(
                'p_id' => $product->id
            )); ?>" target="_blank"><?php
            echo $product->name; ?></a>
             <?php
...
?>
           <a href="<?php
            echo Html::uriquery('mod_product', 'admin_edit', array(
                'p_id' => $product->id
            )); ?>" title="<?php
            _e('Edit'); ?>"><?php
            echo $product->name; ?></a>
...
?>
            ...
            <td><?php
        echo isset($product->masters['ProductCategory']->name) ? $product->masters['ProductCategory']->name : __('Uncategorised'); ?></td>
            <td><?php
        echo Html::input('text', 'i_order[' . $product->id . ']', $product->i_order, 'class="textinput" style="width:20px;"'); ?></td>
...
?>
            ...
            <td><?php
        echo date('y-n-j g:i', $product->create_time); ?></td>
            <td>
<?php
...
?>                     
            ...
            _e('Edit'); ?>"><img style="border:none;" alt="<?php
            _e('Edit'); ?>" src="<?php
            echo P_TPL_WEB; ?>/images/edit.gif"/></a>
<?php
        }
?>       
            </td>
  ...
  <?php
        if (ACL::isAdminActionHasPermission('mod_product', 'admin_delete')) {
?>                   
                <a href="#" onclick="delete_product(<?php
            echo $product->id; ?>);return false;" title="<?php
            _e('Delete'); ?>"><img style="border:none;" alt="<?php
            _e('Delete'); ?>" src="<?php
            echo P_TPL_WEB; ?>/images/cross.gif"/></a>
<?php
        }
?>  
            </td>
