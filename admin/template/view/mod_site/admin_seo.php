...
function createXml(tag){
	$.post("../index.php?_m=mod_sitemap&_a=create_sitemap",{tag:tag},function(msg){
		if(tag=="b"){
		document.getElementById("stbaidu").innerHTML="<a target='_blank' href='../sitemap_baidu.php'>"+"<?php _e('Baidu map'); ?>"+"</a>";;
		}else if(tag=="g"){
			document.getElementById("stgoogle").innerHTML="<a target='_blank' href='../sitemap.php'>"+"<?php _e('Google map'); ?>"+"</a>";
		}
		alert(msg);
	});
}
...
?>
<input type="hidden" name="rurl" value="<?php echo $_REQUEST["rurl"];?>" />
    ...
    <tbody>
       <tr>
		...
		<tr>
            ...
            <a target="_blank" href="http://www.google.com/intl/zh-CN/add_url.html" class="VILinkText"><?php _e('Submit to google');?></a>&nbsp;&nbsp;
            <a target="_blank" href="http://search.help.cn.yahoo.com/h4_4.html" class="VILinkText"><?php _e('Submit to Yahoo');?></a>&nbsp;&nbsp;
        ...
        <tr>
            ...
            <td class="entry">&nbsp;&nbsp;
            <span id="stbaidu"><?php 
            if (file_exists(ROOT."/file/".THISSITE."sitemap_baidu.xml")) {
            	echo '<a target="_blank" href="../sitemap_baidu.php">'.__("Baidu map").'</a>&nbsp;&nbsp;&nbsp;&nbsp;';
            }
			?>
			</span>
			<span id="stgoogle">
			<?php
            if (file_exists(ROOT."/file/".THISSITE."sitemap.xml")) {
            	echo '<a target="_blank" href="../sitemap.php">'.__("Google map").'</a>&nbsp;&nbsp;';
            }
            ?>
            </td>
        </tr>
