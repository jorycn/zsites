...
<table id="sinfoform_table" class="form_table" width="100%" border="0" cellspacing="0" cellpadding="0" style="line-height:24px;">
    ...
    <tbody>
		...
		<tr>
        	...
        	<td class="entry">
            <?php
            echo Html::select('sparam[CURRENCY]', $currencr,CURRENCY, 'class="textselect"');
            ?>
            </td>
        </tr>
		  <tr>
            ...
            <td class="entry">
            <?php
            echo Html::input('text', 'sparam[CURRENCY_SIGN]', CURRENCY_SIGN, 'class="textinput" style="width:60px;"',$sinfo_form);
            ?>
             <img id="answer11" class="title" src="<?php echo P_TPL_WEB; ?>/images/answer1.gif" alt="help" title="<?php _e('eg:U.S. dollar is $');?>"/>
            </td>
        </tr>
		 <tr>
            <td class="label"><?php _e('Area');
			if (SITE_PROVINCE=="SITE_PROVINCE"){
				addconfig(THISSITE."_","SITE_PROVINCE","");
			}
			if (SITE_CITY=="SITE_CITY"){
				addconfig(THISSITE."_","SITE_CITY","");
			}
			if (SITE_MOBILE=="SITE_MOBILE"){
				addconfig(THISSITE."_","SITE_MOBILE","");
			}
			if (SITE_QQ=="SITE_QQ"){
				addconfig(THISSITE."_","SITE_QQ","");
			}
			if (SITE_EMAIL=="SITE_EMAIL"){
				addconfig(THISSITE."_","SITE_EMAIL","");
			}
			if (MESSAGE_VERIFY=="MESSAGE_VERIFY"){
				addconfig(THISSITE."_","MESSAGE_VERIFY",0);
			}
			?></td>
            <td class="entry">
            <select id="b_province" name="sparam[SITE_PROVINCE]"></select>
			<select id="b_city" name="sparam[SITE_CITY]"></select>
			<script language="javascript" type="text/javascript" src="../script/area.js"></script>
			<script language="javascript" type="text/javascript">
				var province = "<?php echo SITE_PROVINCE;?>";
				if (province != "0" && province != "") {
					document.getElementById("b_province").value = province;
					fill_opt(document.getElementById("b_city"), get_citys(province));
				}
				var City = "<?php echo SITE_CITY;?>";
				if (City != "0" && City != "") {
		...
		<tr>
            <td class="label"><?php _e('MOBILE'); ?></td>
            <td class="entry">
            <?php
            echo Html::input('text', 'sparam[SITE_MOBILE]', SITE_MOBILE, 'class="textinput"');
            ?>
            </td>
        </tr>
		<tr>
            <td class="label"><?php _e('QQ'); ?></td>
            <td class="entry">
            <?php
            echo Html::input('text', 'sparam[SITE_QQ]', SITE_QQ, 'class="textinput"');
            ?>
            </td>
        </tr>
		<tr>
            <td class="label"><?php _e('EMAIL'); ?></td>
            <td class="entry">
            <?php
            echo Html::input('text', 'sparam[SITE_EMAIL]', SITE_EMAIL, 'class="textinput"');
            ?>
            </td>
        </tr>
		<tr>
		...
		<tr>
            <td class="label"><?php _e('Message Switch'); ?></td>
            <td class="entry">
            <?php
            echo Html::input('checkbox', 'sparam[MESSAGE_VERIFY]', '1', 
                Toolkit::switchText(MESSAGE_VERIFY, 
                    array('0' => '', '1' => 'checked="checked"')));
            ?>
        ...
        <tr>
            ...
            echo Html::textarea('sparam[SITE_OFFLINE_MSG]', SITE_OFFLINE_MSG, 
                'cols="48" rows="6" class="textinput" style="width:400px;"');
            ?>
            </td>
        </tr>
		...
		<tr>
           ...
           // echo Html::input('hidden', 'banner[id]', $curr_banner->id);
            ...
            echo Html::textarea('param[html]', isset($p_foot['html'])?$p_foot['html']:'', 'rows="8" cols="76" class="textinput" style="width:400px;"')
            ?>
            </td>
        </tr>
		<tr>
		...
		<tr>
			...
			<input type="hidden" value="<?php echo $_REQUEST["rurl"];?>" name="rurl">
            </td>
        </tr>
    </tbody>
</table>
