...
switch($ftype) {
	case 'flash':
		$fext = '*.swf';
	    $fdesc = 'SWF(*.swf)';
		$base_upload_dir = '../file/'.THISSITE.'/flash/';
		break;
	case 'media':
	...
	case 'file':
		$fext = '*.zip;*.rar';
	    $fdesc = 'File(*.zip, *.rar)';
		$base_upload_dir = '../file/'.THISSITE.'/file/';
		break;
	case 'image':
		default:
...
$(document).ready(function() {
	$("#uploadify").uploadify({
		...
		'folder'         : "../file/<? echo THISSITE; ?>/<?php echo $ftype.'|'.$check_admin;?>",
		'queueID'        : 'fileQueue',
		'auto'           : false,
		'multi'          : true,
		'alstr'          : "<?php _e('The queue is full.  The max size is ');?>",
		'cnfstr'         : "<?php _e('Sure to replace the file ');?>",
		'sizeLimit'      : '2097152',//2M = ini_get('memory_limit')
