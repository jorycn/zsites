...
define('UC_CLIENT_VERSION', '1.5.0');	//note UCenter 汾ʶ
...
if(!defined('IN_UC')) {
	...
	if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
		require_once DISCUZ_ROOT.'./uc_client/db_mysql.class.php';
...
class uc_note {
	...
	function updateapps($get, $post) {
		...
		//note дļ
		if(is_writeable($this->appdir.'./config/config.inc.php')) {
			$configfile = trim(file_get_contents($this->appdir.'./config/config.inc.php'));
			$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
			$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '$UC_API');", $configfile);
			if($fp = @fopen($this->appdir.'./config/config.inc.php', 'w')) {
...
//note ʹøúǰҪ require_once $this->appdir.'./config/config.inc.php';
