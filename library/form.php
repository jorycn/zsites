...
class Form {
    ...
    public function genCompareValidate($element_name_1, $element_name_2, $compare = '=', $alert_msg = '') {
        if (empty($alert_msg)) {
        ...
        $this->_validate_script.= 'if (thisForm.elements["' . $element_name_1 . '"].value ' . $compare . ' thisForm.elements["' . $element_name_2 . '"].value)' . "\n" . '{' . "\n" . "\t" . 'alert("' . $alert_msg . '");' . "\n" . "\t" . 'return false;' . "\n" . '}' . "\n";
    }
    ...
    public function genRequiredRichText($instance_name, $alert_msg = '') {
        ...
        $instance_var_name = str_replace(array(
            '[',
            ']'
        ) , array(
            '_',
    ...
    public function genRequiredSelect($element_name, $alert_msg = '') {
        ...
        $element_var_name = str_replace(array(
            '[',
            ']'
        ) , array(
            '_',
...
class Html {
    public static function input($type, $name, $value = '', $extra = '', &$form = false, $validate_type = false, $alert_msg = '') {
    ...
    public static function textarea($name, $value = '', $extra = '', &$form = false, $validate_type = false, $alert_msg = '') {
        $id = self::escSpecialChars($name);
        $extra = ' ' . $extra;
    ...
    public static function select($name, $hash_entry, $selected_value = '', $extra = '', &$form = false, $validate_type = false, $alert_msg = '') {
        $id = self::escSpecialChars($name);
        $extra = ' ' . $extra;
        $html = '';
        if (sizeof($hash_entry) >= 1) {
            ...
            foreach ($hash_entry as $key => $value) {
                ...
                } else if (strval($key) === strval($selected_value)) {
                    $html.= ' selected="selected"';
                }
                $html.= '>' . $value . '</option>';
            }
    ...
    public static function multiselect($name, $hash_entry, $selected_values = array() , $extra = '', &$form = false, $validate_type = false, $alert_msg = '') {
        $id = self::escSpecialChars($name);
    ...
    public static function groupcheck($name, $hash_entry, $checked = array() , $extra = '', $cols = 0, &$form = false, $validate_type = false, $alert_msg = '') {
        $check_idx = 1;
        $html = '';
        if (sizeof($hash_entry) >= 1) {
            foreach ($hash_entry as $key => $value) {
    ...
    public static function groupradio($name, $hash_entry, $checked = false, $extra = '', $cols = 0, &$form = false, $validate_type = false, $alert_msg = '') {
        $check_idx = 1;
        $html = '';
        if (sizeof($hash_entry) >= 1) {
            foreach ($hash_entry as $key => $value) {
...
";
    ...
    public static function adminBar() {
        ...
        $content_entries = & $o_admin_menu_item->findAll("`level` <= ?", array(
            EZSITE_LEVEL
        ...
        if (strpos($_SERVER["PHP_SELF"], '/admin/') !== false) {
            $admin_entries = array(
                'Tools' => array(
                    array(
                        '1',
                        'browser/tinybrowser.php?type=image',
                        'Image Manager'
                    ) ,
                    array(
                        '1',
                        'browser/tinybrowser.php?type=flash',
                        'Flash Manager'
                    ) ,
                ) ,
                'System' => array(
...
class RichTextbox {
    ...
    public function create($create_mode = FCK_CREATE, &$form = false, $validate_type = false, $alert_msg = '') {
        $instance_var_name = str_replace(array(
            '[',
            ']'
        ) , array(
...
class DateTimeInput {
    public static function create($name, &$form, $value = '', $extra = '', $validate_type = false, $alert_msg = '') {
        $html = Html::input('text', $name, $value, $extra, $form, $validate_type, $alert_msg);
